...
- **Every serialized UI reference is optional** (decision #26): the dev may disable or delete any button, label, badge, or template and nothing throws or gets stuck — dereferences are guarded, missing templates `LogError` and skip building, cell/wedge indexing is bounds-checked. A panel opened before `SetInfo` logs one error and refuses, instead of NRE-spamming.
...
- Panels are prefab-authored; references are `[SerializeField]`. No runtime `new GameObject()` UI; dynamic lists instantiate a disabled authored template.
...
| 20 | **Daily Open All is gated behind ads or IAP; IAP enters the package as the `PurchaseIap` hook** (2026-08-20, display rule tightened 0.8.1): two buttons stacked at the mockup's single-button position, **at most one visible, picked by data — IAP config wins over ads, never both** — ads (N rewarded ads via `AdFlow`, progress `X/N` persisted as `OpenAllAdsWatched`, placement `DailyReward_OpenAll`) and IAP; the dev supplies only `openAllAdsRequired` / `openAllIapProductId` / `openAllIapPriceText` (display string, not store-fetched); Open All claims every remaining day, and day-7 completion is `StreakDay == 7` with reset on the next UTC day | The package triggers monetization itself so integration stays "fill 3 fields + wire 1 hook line"; a display-string price matches the consumer's actual habit; one visible button keeps the mockup layout honest |
